            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">{p.label}</p>
...
export default function SharedProfilePage() {
  const { profile, loading, error, updateProfile, uploadAvatar } = useStudentProfile();
  ...
  const displayStudentId = profile?.studentId || null;
  const avatarUrl = profile?.avatar || null;
  const totalEvents = registrations.filter((reg) => reg?.status !== "cancelled").length;
  ...
  return (
    <div className="min-h-screen bg-background-light dark:bg-background-dark">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        {loading && (
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Loading profile…</p>
        )}
        {error && !loading && (
            ...
            <div className="flex-1 min-w-0">
              ...
              {displayStudentId && (
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <p className="text-slate-400 text-sm">Student ID: {displayStudentId}</p>
                </div>
              )}
              {profile?.bio && (
        ...
        <div className="flex flex-wrap gap-4 mb-6">
          <StatCard icon="event_available" label="Total Events" value={totalEvents} />
          <StatCard icon="workspace_premium" label="Certificates" value={0} />
        </div>

        {/* Social Links */}
        <SocialLinksSection socialLinks={socialLinks} onEditClick={() => setIsEditing(true)} />
