...
export default function EventLoader({ activeEventId, onLoadEvent, loading, error }) {
  ...
  return (
    <motion.div
      ...
      className="rounded-2xl bg-white dark:bg-slate-900 p-6 shadow-lg ring-1 ring-slate-100 dark:ring-slate-800"
    >
      ...
      <div className="space-y-3">
        <div className="space-y-1.5">
          ...
          {listLoading ? (
            <div className="flex h-11 items-center rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400">
              Loading events…
            </div>
          ) : listError ? (
          ...
          {showFallback && (
            <form onSubmit={handleFallbackSubmit} className="mt-2 flex gap-2">
              <input
                ...
                className="flex-1 rounded-lg border border-slate-200 bg-slate-50 px-2 py-1.5 text-xs text-slate-900 outline-none focus:border-indigo-400 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-50"
              />
              <button
                type="submit"
                disabled={!fallbackId.trim() || loading}
                className="rounded-lg bg-slate-200 px-2 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-300 disabled:opacity-50 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600"
              >
                Load
              </button>
            </form>
