...
const ROLE_LABELS = {
  ...
  club_leader: { label: "Club Leader", cls: "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300" },
    ...
    <div className="p-6 sm:p-8 max-w-7xl mx-auto w-full">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
        {loading ? (
          ...
          <div className="p-10 text-center text-sm text-slate-500 dark:text-slate-400">No users found.</div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {items.map((u) => (
                  <tr key={u._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {u.studentId && (
                            <p className="text-[11px] text-slate-500 dark:text-slate-400">ID: {u.studentId}</p>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-3.5">
                      <p className="text-slate-700 dark:text-slate-200">{u.department || "—"}</p>
                      {u.year && <p className="text-[11px] text-slate-500 dark:text-slate-400">Year {u.year}</p>}
                    </td>
                    <td className="px-5 py-3.5">
                    ...
                    <td className="px-5 py-3.5">
                      {loadingId === u._id + "_role" ? (
                        ...
                        <select
                          ...
                          value={u.role}
                          onChange={(e) => handleRoleChange(u._id, e.target.value, u.role, u.name)}
                        >
        ...
        {total > 0 && (
          <div className="flex items-center justify-between border-t border-slate-200 px-5 py-3 text-xs text-slate-500 dark:border-slate-700 dark:text-slate-400">
            <p>Page {page} of {pages} · {total} users total</p>
